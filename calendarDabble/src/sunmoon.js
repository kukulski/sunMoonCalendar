...
function localSideralTime(mjd, glong) {
//
...
function equatorialSunPosition(t) {
//
...
function equatorialMoonPosition(t) {
//
...
function equatorialToSinAlt(objpos, mjd, glong, sglat, cglat) {
    var ra = objpos[2];
    var dec = objpos[1];
    // hour angle of object
    var rads = 0.0174532925;
    var tau = 15.0 * (localSideralTime(mjd, glong) - ra);
    // sin(alt) of object using the conversion formulas
    var rval = sglat * Math.sin(rads * dec) + cglat * Math.cos(rads * dec) * Math.cos(rads * tau);
    return rval;
}
...
//
    ...
    var objpos = (iobj == 1) ?
		objpos = equatorialMoonPosition(t):
		objpos = equatorialSunPosition(t);
    return equatorialToSinAlt(objpos, mjd, glong, sglat, cglat);
}
